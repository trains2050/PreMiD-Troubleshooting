...
function askQuestion() {
    ...
    if (number == 0) main.innerHTML += `${question[0]}<br>`
    else main.innerHTML += `<br>${question[0]}<br>`
    ...
    question[1].forEach((action, index, array) => {
        if (action.deadend) {
            buttons.innerHTML += `<a id="action${index}" onclick="deadend('${action.deadend}', ${index}, this)">${action.text}</a>`
        } /*else if (action.followup) {
            buttons.innerHTML += `a`
        }*/ else {
            buttons.innerHTML += `<a id="action${index}" onclick="followup(${number + 1}, ${index}, this)">${action.text}</a>`
        }
    })
...
function deadend(deadend, index, element) {
    ...
    main.innerHTML = `<b>${main.innerHTML}</b><br>${deadend}`

    // Add to searchParams
    if (!params.has(number) && element?.parentElement ) params.append(number, index)
